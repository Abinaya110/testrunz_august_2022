...
const getaccess = async function (req, res, next) {
  ...
  try {
    ...
    if(role==="admin"){
      ...
      const updatedContent = await User.find({ $and: [{$or:[{role:"teacher"},{role:"student"}] }, { department:department }, { instituteName:instituteName }]});
      // const updatedContent = await User.find({$or:[{role:"teacher"},{role:"student"}] });
      res.json(updatedContent);
    }
    if(role==="teacher"){
      const updatedContent = await User.find({ $and:[{role:"student"},{department:department },{instituteName:instituteName},{ labtype: { $in: req.body.lab} }]});
      res.json(updatedContent);
    }
