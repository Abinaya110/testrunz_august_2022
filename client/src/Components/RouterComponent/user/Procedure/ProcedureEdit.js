...
const App = (props) => {
  ...
  const duplicate = (e) => {
    ...
    if (!titleRef.current.value ){
      ...
      instituteval = instituteRef.current.value ? instituteRef.current.value : "";
      semval = semesterRef.current.value ? semesterRef.current.value : "";
      console.log("all",experimentval ,labval ,Departmentval ,yearval ,instituteval ,semval )
      setOpen(true);
  ...
  const Results = (props) => (
    <Card className="blocks" style={{ backgroundColor: "#E7E9EB" ,position:"absolute",top:"120px",zIndex:10,paddingLeft:10,paddingRight:10}}>
      <CardContent>
        ...
        <br />
        <br />
        <p>Institute: {state1.institute ?? "Institute"}</p>
        <input
          className="institute"
  ...
  const Search = () => {
    ...
    return (
      <div>
        ...
        {showResults ? 
        <div>
          {state1.category==="institute" && <Results  onclick={onClick}/>}
  ...
  const handleSave = (e) => {
    ...
    instituteval = instituteRef.current.value ? instituteRef.current.value : state1.institute;
